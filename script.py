...
scaler_fit = StandardScaler().fit(features)
X = scaler_fit.transform(features)
...
print("\nRidge:")
print("Training Score:", f1_score(y_train, y_pred_train2))
print("Testing Score:", f1_score(y_test, y_pred_test2))
...
gscv = GridSearchCV(estimator=clf_gs, param_grid=tuning_C, scoring='f1', cv=5)
gscv.fit(X_train, y_train)

## 12. Optimal C value and the score corresponding to it
print("Optimal C value:", gscv.best_params_)
print("Score:", gscv.best_score_)

## 13. Validating the "best classifier"
clf_best_ridge = LogisticRegression(C=gscv.best_params_["C"])
clf_best_ridge.fit(X_train, y_train)
y_pred_best = clf_best_ridge.predict(X_test)
print("\nBest f1 score on optimal C:", f1_score(y_test, y_pred_best))
